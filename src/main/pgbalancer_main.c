...
PgpoolMain(bool discard_status, bool clear_memcache_oidmaps)
{
	...
	pgbalancer_logger_pid = SysLogger_Start();
		...
		if (first)
		{
			...
			if (STREAM)
			{
				for (i = 0; i < NUM_BACKENDS; i++)
				{
					...
					{
						ereport(LOG,
								(errmsg("pgbalancer_main: invalid node found %d", i)));
...
reaper(void)
{
	...
	while ((pid = pool_waitpid(&status)) > 0)
	{
		...
		else if (pid == pgbalancer_logger_pid)
		{
			if (restart_child)
			{
				pgbalancer_logger_pid = SysLogger_Start();
...
kill_all_children(int sig)
{
	...
	if (sig == SIGHUP)
	{
		...
		if (pgbalancer_logger_pid > 0)
			kill(pgbalancer_logger_pid, sig);
	}
}
...
read_status_file(bool discard_status)
{
	...
	/*
	 * If discard_status is true, unlink pgbalancer_status and do not restore
	 * previous status.
	 */
	if (discard_status)
	{
...
write_status_file(void)
{
	 ...
	 * file. So pgbalancer_status will always reflect the last set of nodes to
...
pool_signal_logrotate(void)
{
	...
	{
		ereport(DEBUG1,
				(errmsg("sending SIGUSR1 to logger process with PID:%d to rotate log file", pgbalancer_logger_pid)));

		if (kill(pgbalancer_logger_pid, SIGUSR1) != 0)
			ereport(ERROR,
					(errmsg("process log rotation request failed"),
					 errdetail("failed to send SIGUSR1 to logger process with PID:%d", pgbalancer_logger_pid)));
	}
	else
	{
		ereport(ERROR,
				(errmsg("process log rotation request failed"),
				 errdetail("logger process PID:%d is not valid", pgbalancer_logger_pid)));
	}
}
